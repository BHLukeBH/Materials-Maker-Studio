...
HERE"
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 flex flex-col items-center">
                        ...
                        {isGenerated ? (
                            <div className="bg-white p-4 shadow-sm border border-slate-300">
                                <div className="grid grid-cols-15 gap-0 border-t border-l border-slate-800" style={{ display: 'grid', gridTemplateColumns: 'repeat(15, 1fr)' }}>
                                    {grid.map((row, rIdx) => (
                                        row.map((letter, cIdx) => (
                                            <div 
                                                key={`${rIdx}-${cIdx}`} 
                                                className={`
                                                    w-6 h-6 flex items-center justify-center text-xs font-mono border-b border-r border-slate-800
                                                    ${showAnswers && isCellInAnswer(rIdx, cIdx) ? 'bg-yellow-200 font-bold' : ''}
                                                `}
                                            >
                                                {letter}
                                            </div>
            ...
            {isGenerated && (
                <div className="print-only w-full h-full bg-white">
                    ...
                    <div className="w-full h-[95vh] page-break p-8 flex flex-col items-center bg-white">
                        ...
                        {/* Grid */}
                        <div className="mb-8 border-2 border-slate-800">
                             {grid.map((row, rIdx) => (
                                <div key={rIdx} className="flex">
                                    {row.map((letter, cIdx) => (
                                        <div key={cIdx} className="w-10 h-10 border border-slate-300 flex items-center justify-center font-mono text-lg">
                                            {letter}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="w-full max-w-2xl">
                            <h3 className="font-bold border-b-2 border-slate-800 mb-2">Find these words:</h3>
                            <div className="grid grid-cols-3 gap-4">
                                {config.words.map(w => (
                                    <div key={w} className="font-mono text-lg">{w}</div>
                                ))}
                            </div>
                        </div>
                                ...
                                <div key={rIdx} className="flex">
                                    {row.map((letter, cIdx) => (
                                        <div 
                                            key={cIdx} 
                                            className={`
                                                w-10 h-10 border border-slate-300 flex items-center justify-center font-mono text-lg
                                                ${isCellInAnswer(rIdx, cIdx) ? 'bg-slate-300 font-bold' : ''}
                                            `}
                                        >
                                            {letter}
                                        </div>
